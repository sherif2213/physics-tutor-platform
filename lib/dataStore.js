...
import {
  cacheAll, cacheOne, getAllCached, getCachedByStudent, queueMutation, deleteCached,
} from './offlineDb';
...
export async function upsertStudent(student) {
  const row = { ...student, id: student.id || crypto.randomUUID(), updated_at: new Date().toISOString() };
...
export async function softDeleteStudent(id) {
  ...
  const { groups, ...existingClean } = existing;
  const row = { ...existingClean, id, deleted_at: new Date().toISOString() };
  await cacheOne('students', row);
  await queueMutation({ table: 'students', op: 'upsert', payload: { ...row, __conflictKey: 'id' } });
  await logActivity('حذف طالب', { id, name: existingClean.full_name });
  if (isOnline()) await runSync();
}
...
export async function deleteGroup(id) {
  await deleteCached('groups', id);
  await queueMutation({ table: 'groups', op: 'delete', payload: { id } });
  if (isOnline()) await runSync();
  return true;
}
...
export async function getSettings() {
  ...
  return { center_name: 'الشاهين للفيزياء', phone: '', address: '', logo_url: null };
}
