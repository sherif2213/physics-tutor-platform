          ...
          id, student_id: studentId, month, lesson_number: lessonNumber,
              present: !currentlyPresent, marked_at: new Date().toISOString(),
                  ...
                  await cacheOne('attendance', row);
                    const { id: _localId, ...remoteRow } = row;
                      await queueMutation({ table: 'attendance', op: 'upsert', payload: { ...remoteRow, __conflictKey: 'student_id,month,lesson_number' } });
                        if (isOnline()) runSync();
                          return row;
                          }
          ...
          id, student_id: studentId, month, paid: !currentlyPaid,
              amount: amount || null, paid_at: !currentlyPaid ? new Date().toISOString() : null,
                  ...
                  await cacheOne('payments', row);
                    const { id: _localId, ...remoteRow } = row;
                      await queueMutation({ table: 'payments', op: 'upsert', payload: { ...remoteRow, __conflictKey: 'student_id,month' } });
                        if (isOnline()) runSync();
                          return row;
                          }
